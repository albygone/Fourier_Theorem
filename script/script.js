...
function cambiaArmonicheTri(value) {
    ...
    for (let i = 0, j = 1; i < num; i++, j += 2) {
        ondeTri.push(new ondaTri(Math.PI, 100, to, "#" + Math.floor(Math.random() * 16777215).toString(16), 0.005, canvas, j));
        ondeTri[ondeTri.length - 1].calcola();
    }
    ondaRisultanteTri = new ondaRis(Math.PI, 80, to, "#000000", 0.005, canvas, ondeTri);
...
function cambiaArmonicheQua(value) {
    ...
    ondeQua = [];
    for (let i = 0, j = 1; i < num; i++, j += 2) {
        ondeQua.push(new ondaSin(Math.PI, 100, to, "#" + Math.floor(Math.random() * 16777215).toString(16), 0.005, canvas, j));
    ...
    ondaRisultanteQua = new ondaRis(Math.PI, 80, to, "#000000", 0.005, canvas, ondeQua);
